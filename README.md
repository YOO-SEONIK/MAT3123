...
만약 큰 오차가 보이면 (a) 표준화 스케일 적용 위치/축 오류, (b) 로그 기반 체인룰의 $ln(10)$ 계수 누락, (c) 표준화/원단위 혼용, (d) 너무 큰 $\epsilon$로 인한 비선형 영향 등을 점검해야 한다. 
...
먼저 _turning_points(y, $\epsilon$) 함수가 시계열에서 기울기 부호가 바뀌는 극값만 추출해 평탄 구간을 제거하고, $\epsilon$ 값(절대 차 컷오프)을 이용해 미세한 노이즈 요동을 제거한다. 이렇게 하면 불필요한 작은 사이클이 레인플로우 단계에서 중복 계산되는 문제를 방지할 수 있다.

다음으로 rainflow(series, $\epsilon$)는 ASTM 규칙 기반의 레인플로우 사이클 계수 알고리즘을 구현한다. 턴닝 포인트 배열을 스택으로 처리하여 범위(range), 평균(mean), 카운트(count=1.0 또는 0.5)를 구한다. 이때 1.0은 완전주기, 0.5는 반주기를 의미하며 출력은 (range, mean, count)의 배열, 각 원소가 하나의 피로 사이클에 해당한다.

핵심 함수 miner_from_strain_eN($σ_{series}$, $T_{in, series}$, …)는 서로게이트가 예측한 응력 $σ(t)$과 입력 온도 $T_{in}(t)$을 이용해, 온도 의존 탄성계수 $E(T)=E_{25}\exp\(-\beta\(T-25))$를 계싼하고 변형률 $\epsilon(t) = \dfrac{\sigma}{E(T)}$를 구한다.

그다음 레인플로우 결과에서 진폭 $\epsilon_a = range/2$를 얻고, 고무용 $ε–N$ 관계식 $N_f=(\dfrac{\epsilon_0}{\epsilon_a})^b$으로 각 사이클의 수명을 계산한다.

피로한계 $epsilon_e$(진폭 컷)를 두면 그보다 작은 사이클은 손상에서 제외되고, $Miner$ 합산으로  $D=\Sigma \dfrac{count}{N_f}$로 누적 손상을 반환한다($D≈1$을 파손 한계로 해석)
