...
이 단계는 속 빈 원통(고무 링)의 반지름 방향 정상상태 온도분포 $T(r)$를 구한다. 축대칭, 축방향 열흐름 무시, 내부 생성열 0, 고체 열전도도 $k$ 상수 가정에서 지배식은 $\dfrac{\mathrm{d}}{\mathrm{d}r}\!\bigl(r\,\dfrac{\mathrm{d}T}{\mathrm{d}r}\bigr)=0$이고 해는 $T(r)=C_1\ln r + C_2$이다. 내부/외부 표면에는 대류 경계가 적용된다: $r=r_i$에서 $-\,k\,\dfrac{\mathrm{d}T}{\mathrm{d}r}=h_i\,(T-T_{\infty,i})$, $r=r_o$에서 $-\,k\,\dfrac{\mathrm{d}T}{\mathrm{d}r}=h_o\,(T-T_{\infty,o})$. 여기서 $\dfrac{\mathrm{d}T}{\mathrm{d}r}=\dfrac{C_1}{r}$이므로 $C_1, C_2$에 대한 선형 $2\times2$ 시스템 $\mathbf{M}[C_1\ C_2]^{\!\top}=\mathbf{b}$로 정리된다. $\mathbf{M}=\begin{bmatrix}-\dfrac{k}{r_i}-h_i\ln r_i & -h_i\\[2pt]-\dfrac{k}{r_o}-h_o\ln r_o & -h_o\end{bmatrix}$, $\mathbf{b}=\begin{bmatrix}-\,h_iT_{\infty,i}\\[2pt]-\,h_oT_{\infty,o}\end{bmatrix}$. 선형계를 풀어 얻은 $C_1,C_2$로 $r\in[r_i,r_o]$ 구간을 균일 격자 $N_r$로 샘플링하여 $T(r)=C_1\ln r + C_2$를 계산한다. 출력은 반지름 벡터 $r\,[\mathrm{m}]$와 온도 벡터 $T\,[^\circ\mathrm{C}]$이며(입력 예: $k\,[\mathrm{W}/\mathrm{m}\!\cdot\!\mathrm{K}],\ h_i,h_o\,[\mathrm{W}/\mathrm{m}^2\!\cdot\!\mathrm{K}]$), 본 단계에서는 $k$와 물성을 상수로, 복사·비선형 효과는 무시한다.
