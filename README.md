...
torch.optim을 쓰지 않고 순수 SGD 스텝을 직접 구현해 각 epoch마다 loss.backward()로 기울기를 계산하고, 모든 파라미터에 대해 L2 weight decay(λ·p) 를 더해 적용한 뒤, p ← p − lr·grad 로 직접 갱신한다.
...
학습이 완료되면, 예측값 $\hat{y}{log}$을 원공간으로 되돌려 $\hat{y}=10^{\hat{y}{log}}$로 변환하고, 테스트셋에서의 RMSE(단위: Pa) 를 계산해 최종 성능을 보고한다.
