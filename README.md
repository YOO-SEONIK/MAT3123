...
이 단계는 시간축 $t$와 온도 $T_{in}(t)$, 서로게이트가 예측한 응력 $sigma(t)$, 재료 상수($E_25$, $\beta$)를 받아 $ε–N$ 모델로 누적손상 $D$와 예상 수명 life_hours를 계산한다. 
...
$Miner$ 합산식 $D=\Sigma \dfrac{count}{N_f}$으로 누적 손상을 구하고, 기록 길이를 record_hours = \dfrac{t_{end}-t_{start}+\Delta t}{3600.0}로 환산하여 예상수명을 life_hourts = \dfrac{record-hours}{max(D, ε)}로 외삽한다(분모 0 방지를 위해 작은 하한 사용)
...
함수 내부에서는 $E(T)$로부터 변형률 $ε(t)$을 만들고, rainflow로 사이클 진폭$ε_a$ 및 $count$를 계수한 뒤, $ε–N$ 관계 $N_f=(\dfrac{ε_0}{ε_a})^b$와 $Miner$ 합산으로 $D$를 계산한다. 

출력 로그는 (1) $\sigma_N$의 분포 요약(min, max, std), (2)기록 길이 record_hours, (3) 손상 $D$와 외삽 수명 life_hours를 보여준다. 

$D$ 약 1이 설계 한계, life_hours는 record_hours를 $D$로 나눈 외삽치이며, $ε_e$를 높이면 미세 사이클이 무시되어 수명이 길어지고, 반대로 낮추면 더 보수적인 결과가 나온다.
