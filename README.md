...
모든 지표는 여러 샘플을 무작위로 뽑아(예: $n=256$) 평균±표준편차로 요약하여 지역적 편차도 함께 본다. 해석 팁: 표준화 민감도는 모델 내부의 상대적 의존도를, $\dfrac{\mathrm{d}σ}{\mathrm{d}x}$는 절대 하중 기여도를, 탄력도는 운영 퍼센트 변화 대비 응력 퍼센트 변화를 보여준다. 값이 큰 입력은 모델과 물리 모두에서 중요한 조절 레버일 가능성이 높으며, 데이터 수집 정밀도(센서 정확도)와 운전 제어 우선순위를 정하는 근거가 된다. 단, 이는 선형 근사(국소) 이므로 큰 변화나 학습 분포 밖에서는 해석을 보수적으로 적용해야 한다.
