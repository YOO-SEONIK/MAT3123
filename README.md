...
다음으로 rainflow(series, $\epsilon$)는 ASTM 규칙 기반의 레인플로우 사이클 계수 알고리즘을 구현한다.
턴닝 포인트 배열을 스택으로 처리하여 범위(range), 평균(mean), 카운트(count=1.0 또는 0.5)를 구한다.
이때 1.0은 완전주기, 0.5는 반주기를 의미한다.
출력은 (range, mean, count)의 배열이며, 각 원소가 하나의 피로 사이클에 해당한다.
