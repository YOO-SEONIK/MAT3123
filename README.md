...
## 15. 변형률 진단: 시나리오 생성 → σ(t)· $p_{pass}(t)$ → ε(t) 분석(통계/플롯)
...
## 16. ε–N 수명 계산 함수: σ(t) → ε(t) → 레인플로우 → Miner 손상 → 수명[h]
...
<img width="260" height="103" alt="image" src="https://github.com/user-attachments/assets/2bbb8635-39fb-4177-9cc7-b015fbd06f0f" />

<img width="785" height="65" alt="image" src="https://github.com/user-attachments/assets/567b9a69-639a-4d67-ba22-ddf12004d156" />

# Risk-Life Scenario Map

## 18. 위험 반영 리포트: 수명·리스크 동시 요약 + 완화 게이팅 재산정

이 단계는 앞선 계산 결과를 한눈에 요약하고, 분류기 확률 $p_{pass}(t)$을 이용해 위험 구간 일부를 제거한 완화 게이팅(relaxed gating) 시나리오에서 수명/손상을 다시 계산한다. 목표는 너무 보수적인 운영 가정을 완화했을 때의 현실적(운영가능) 수명 상향 여지를 정량적으로 파악하는 것이다.

먼저 REG에 저장된 핵심 지표중 수명/손상 요약: life_hours, miner_D, record_hours / 리스크 요약: risk_rate(위험 시간 비율), ppass_min(최소 합격확률), tau_star(운영 임계값) 를 정리 출력한다.

그 다음 완화 게이팅 모드를 선택한다.

quantile 모드는 $p_{pass}(t)$ 의 하위 q 구간만 제외한다(기본: 하위 10%) 운영상 가끔 발생하는 최저 신뢰 구간을 계획적으로 건너뛴다(회피 운전, 인터록, 램프업 지연 등)는 해석과 일치한다.

floor 모드는 $p_{pass}(t) < τ_{lo}$만 제외한다(기본 $τ_{lo} = 0.5τ^{*}$) 이는 본래의 임계값보다 느슨한 하한선을 두어, 극단적 저신뢰 시점만 제거하는 방식이다.

이때, 이미 15단계에서 계산한 $σ(t)$, $p_{pass}(t)$ 가 있으면 사용하고, 없으면 동일한 입력 생성 로직으로 즉석 재계산한다(_ensure_scenario). 선택한 게이팅 규칙으로 keep 마스크를 만든 뒤, 남은 시점들만 모아 compute_life_eN_from_sigma(...)로 수명/손상 재평가를 수행한다. 표본이 너무 적으면(3점 미만) 결과 신뢰성이 떨어지므로 $∞$ 처리한다.

마지막으로 [RELAX-GATED] 라벨로 완화 게이팅 후 수명/손상/사용 기록시간을 함께 출력한다. 

이를 통해 운영 전략(회피·완화) 적용 시 수명 상승 폭, 손상 저감효과, 실제 운전 가능 시간 손실: 사용된 record_used(게이팅으로 제외된 시간 비율 간접 확인)를 판단할 수 있다.

정리하면, 본 단계는 (요약 리포트) 수명·손상·리스크 지표를 한 번에 보여주고, (완화 게이팅 재산정) 운영상 합리적인 회피 전략을 반영했을 때의 현실적 수명 시나리오를 제시한다. 이는 후속 의사결정—예: 인터록 조건, 경보 임계, 램프업/쿨다운 정책, 유지보수 간격 재설계—에 바로 활용할 수 있는 실행 가능한 근거를 제공한다.

<img width="762" height="65" alt="image" src="https://github.com/user-attachments/assets/409732a3-e361-4743-bb59-93567fc2792e" />

## 19. 시나리오 스윕(온도 레벨 × 노이즈 세기) 지도: 표 요약 + 수명 맵(로그 컬러)

이 단계는 평균 유입온도 ${T_m}$과 온도 노이즈 진폭을 격자로 바꿔가며 대량의 시나리오를 생성하고, 각 시나리오에서 응력–분류–피로 수명을 일괄 계산해 표와 수명 맵으로 정리한다. 

구체적으로 $T_m∈{60,65,70,75,80} ^{∘}C$, $noise∈{10,12.5,15,17.5,20} ^{∘}C$를 조합하고, 시계열 조건은 $\Delta t = 2s$, 총 $6h$, 1차 스무딩 $τ=12 s$, 고정 시드로 재현성을 확보한다. 

각 격자점마다 scenario_noise로 $T_{in}(t)$를 만든 뒤 run_scenario_and_risk로 $σ(t)$, $p_{pass}(t)$, risk rate를 구하고, 이어 compute_life_eN_from_sigma로 Miner 손상 $D$와 수명 life_hours를 산출한다(ε–N 파라미터는 REG에 저장된 $ε_0$, $b$, $ε_e$를 사용)

결과는 ($T_m$, noise, life_h, life_d, D, risk_rate, min$p_{pass}$) 열 구조의 DataFrame으로 모아 가독성 포맷으로 출력하고 REG["df_sweep"]에 저장해 후속 분석에 재사용한다. 

시각화는 x축 $T_m$, y축 noise, 마커 색을 수명(일)으로 하는 산점도이며, 수명 값의 동적 범위가 크므로 하한 $10^{−6}$을 둔 LogNorm 컬러 스케일을 적용해 한 톤 뭉침을 방지하고 컬러바 라벨은 “Life [days] (log scale)”로 표기한다. 

이 맵은 온도 레벨과 교란 세기에 따른 수명 민감도 지형을 직관적으로 보여주며, 일반적으로 동일 $T_m$에서 noise가 커지면 레인플로우 진폭 빈도 증가로 $D$가 커져 수명이 단축되고, 동일 noise에서 $T_m$상승은 $E(T)$ 감소를 통해 같은 $σ$에서도 $ε$을 키워 수명을 줄이는 경향을 확인할 수 있다. 

아울러 risk rate와 min $p_{pass}$는 운영·인터록 관점의 보조 지표로, 수명이 충분하더라도 리스크가 높은 구간은 램프업 지연, 냉각 여유 확보 같은 운영 정책 보완이 필요함을 시사한다. 
