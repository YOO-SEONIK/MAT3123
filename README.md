...
먼저 학습 입력(X_train)에서 무작위로 일부 샘플($n=64$)을 선택하고, 이를 표준화→역변환하여 원단위 입력 $X_{0}$을 얻는다. 각 입력 피처 $x_j$에 대해 ±0.1%의 미세한 곱셈 변화를 주어 $x_j^{(+)} = x_j(1+\varepsilon), \qquad
x_j^{(-)} = x_j(1-\varepsilon^{-1}$
...
먼저 _turning_points(y, $ε$) 함수가 시계열에서 기울기 부호가 바뀌는 극값만 추출해 평탄 구간을 제거하고, $ε$ 값(절대 차 컷오프)을 이용해 미세한 노이즈 요동을 제거한다. 이렇게 하면 불필요한 작은 사이클이 레인플로우 단계에서 중복 계산되는 문제를 방지할 수 있다.
...
반환값은 길이가 $len(T_{in})$인 1D $numpy$ 배열 $\hat{\sigma}$ $[Pa]$이며, 이후 단계에서 rainflow -> $ε–N$  -> Miner 손상 및 수명 계산에 바로 사용된다.
