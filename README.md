...
$Miner$ 합산식 $D=\Sigma \dfrac{count}{N_f}$으로 누적 손상을 구하고, 기록 길이를 record_hours = $\dfrac{t_{end}-t_{start}+\Delta t}{3600.0}$ 로 환산하여 예상수명을 life_hours = $\dfrac{record__hours}{max(D, ε)}$ 로 계산한다(분모 0 방지를 위해 작은 하한 사용)
